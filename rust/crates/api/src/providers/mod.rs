...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    if let Some(limit) = configured_model_token_limit(model) {
        return Some(limit);
    }
    let canonical = resolve_model_alias(model);
...
fn configured_model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    let cwd = std::env::current_dir().ok()?;
    let config = runtime::ConfigLoader::default_for(cwd).load().ok()?;
    let max_context = config.feature_config().max_context_for_model(model)?;
    let default_max_output = if model.contains("opus") {
        32_000
    } else {
        64_000
    };
    Some(ModelTokenLimit {
        max_output_tokens: default_max_output.min(max_context),
